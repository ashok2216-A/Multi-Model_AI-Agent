...
from utils import get_response_from_api, get_vision_response, extract_text_and_images_with_mapping, save_text_image_mappings, find_relevant_images_vector
        ...
        if 'processed_pdf' not in st.session_state or st.session_state.processed_pdf != uploaded_file.name:
            with st.spinner('Processing PDF...'):
                try:
                    ...
                    if hasattr(st.session_state, 'vector_store_images'):
                        del st.session_state.vector_store_images
                    ...
                    # Extract text and images, store in vector DB
                    mappings, vector_store_images = extract_text_and_images_with_mapping(temp_path, embeddings)
                    # Get text
                    ...
                    st.session_state.vector_store_images = vector_store_images
            ...
            with st.expander(f"Images ({len(st.session_state.text_image_mappings)})"):
                for mapping in st.session_state.text_image_mappings[:]:
                    if 'image_base64' in mapping:
                        import io
                        st.image(io.BytesIO(base64.b64decode(mapping['image_base64'])), caption=f"Page {mapping['page_number']}")
        ...
        if "images" in msg:
            for img in msg["images"][:2]:
                if img.get('image_base64'):
                    st.image(io.BytesIO(base64.b64decode(img['image_base64'])), 
    ...
    with st.chat_message("assistant"):
        with st.spinner("🤔 Thinking..."):
            try:
                ...
                if response:
                    st.write(response)
                    # Only show relevant images for PDF-related queries
                    images = None
                    if hasattr(st.session_state, 'vector_store_images') and 'words' in locals() and words.intersection(pdf_keywords):
                        images = find_relevant_images_vector(user_content, st.session_state.vector_store_images)
                        if images:
